...
const tagsName = route.query.tagsName;
const query = ref<ArticleQuery>({
    current: 1,
    size: 5,
    tagsId: Number(route.query.id!)
})
...
<template>
    <div class="pb-10 my-0 mx-auto">
        <div class="flex items-center   w-fit bg-black p-1 rounded-md cursor-pointer text-sm font-semibold tag"
            @click="callback">
            <svg-icon type="mdi" :path="mdiPound" class="w-4 mr-1" />
            <span>
                {{ tagsName }}
            </span>
            <span class="ml-2 p-1 bg-white text-black rounded-sm tag-count ">
                {{ articleCount }}
            </span>
        </div>
        <div class="min-w-[800px]">
            <div v-if="articleCount > 0" v-for="item in showArticleList"
